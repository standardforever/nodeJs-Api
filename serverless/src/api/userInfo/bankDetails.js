...
// personal info
app.put("/api/bank-details", authenticate, asyncHandler(async (req, res) => {
    try {
      const data = req.body;
      ...
      return res.json({
        message: "User Bank Info Added Successfully",
        id: req.user_id,
      });
    } catch (error) {
      console.error("Error while Adding user bank info:", error);
      res
        .status(400)
        .json({ message: "Error while Adding user Bank detail" });
    }
  })
);
